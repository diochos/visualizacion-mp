// consumo-por-linea.js — usa sessionStorage (vmpsession_v1) y renderiza filtros, KPIs y tabla
...
(function () {
  ...
  function buildTimeline(rows) {
    timeline.innerHTML = "";
    const months = uniqueBy(rows.map((r) => ({ mk: monthKey(r.Fecha) })), "mk").filter(Boolean).sort();
    if (months.length === 0) {
  ...
  function render() {
    ...
    if (window.jQuery && jQuery.fn && jQuery.fn.dataTable) {
      if (jQuery.fn.dataTable.isDataTable("#tablaLinea")) {
        jQuery("#tablaLinea").DataTable().destroy();
      }
      jQuery("#tablaLinea").DataTable({
        pageLength: 25,
        autoWidth: false,
        order: [[3, "desc"]],
  ...
  (async () => {
    ...
    if (!sess || !Array.isArray(sess.rows) || sess.rows.length === 0) {
      tbody.innerHTML =
        `<tr><td colspan="5" class="muted">No hay datos en la sesión. Ve a <a href="index.html">Concentrado</a> y sube un archivo.</td></tr>`;
      return;
    }

    if (fileTag) fileTag.textContent = sess.filename || "Sesión restaurada";

    // asegurar claves esperadas por si los encabezados variaron en el XLSX
    state.rows = sess.rows.map((r) => ({
      MateriaPrima: r.MateriaPrima ?? r["Materia Prima"] ?? r.ArticuloDescripcion ?? r["Nombre de artículo"] ?? "",
      CantidadTeorica: Number(r.CantidadTeorica ?? r["Cantidad Teórica"] ?? 0),
      CantidadReal: Number(r.CantidadReal ?? r["Cantidad Real"] ?? 0),
      Merma: Number(r.Merma ?? 0),
      Linea: r.Linea ?? r["Línea"] ?? r.Recurso ?? "",
    ...
    selCategoria.addEventListener("change", () => {
      const cat = selCategoria.value || "";
      const subs = cat
        ? uniqueBy(state.rows.filter((r) => (r.CategoriaMP || "") === cat), "SubcategoriaMP")
            .filter((s) => s && s.trim() !== "")
        : [];
